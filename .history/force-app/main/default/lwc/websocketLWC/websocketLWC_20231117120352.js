...
export default class WebsocketLWC extends LightningElement {
    ...
    async connectedCallback(){
      if(this.requestToken ){
        let response;
        if(this.accessTokenObject === undefined)
          let response = await getAccessToken({apiKey:this.apiKey ,apiSecret:this.apiSecret, requestToken:this.requestToken});
        console.log(response);
        this.accessTokenObject = JSON.parse(response);
        this.webSocket = new WebSocket(this.url+this.accessTokenObject.public_access_token);
    ...
    processByteMessage(message) {
			...
			arrayBufferPromise.then((data) => {
				...
				while (this.position != l) {
				 ...
				 processQuotePacket(dvu) {
					...
					console.log("traded: " + dvu.getInt8(this.position + 12, true));
					console.log("Mode: " + dvu.getInt8(this.position + 13, true));
					console.log("last_traded_quantity " + dvu.getInt32(this.position + 14, true));
					console.log("average_traded_price: " + dvu.getFloat32(this.position + 18, true));
					console.log("volume: " + dvu.getInt32(this.position + 22, true));
					console.log("total_buy_quantity: " + dvu.getInt32(this.position + 26, true));
					console.log("total_sell_quantity: " + dvu.getInt32(this.position + 30, true));
				 ...
				 processFullPacket(dvu) {
					...
					for (let i = 0; i < 5; i++) {
						...
						console.log("sell_quantity: " + dvu.getInt32(depththis.position + 4, true));
						...
						console.log("buy_price: " + dvu.getFloat32(depththis.position + 12, true));
						console.log("sell_price: " + dvu.getFloat32(depththis.position + 16, true));
						console.log("\n");
          ...
          let xAxisValue = dvu.getInt32(this.position + 34, true);
